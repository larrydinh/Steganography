...
from streamlit_app.ui_helpers import apply_global_styles, render_page_header
...
st.set_page_config(page_title="Decode Message", page_icon="🔓", layout="wide")
apply_global_styles()
...
render_page_header(
    "Decode Message",
    "Upload a stego image or retrieve one by code, then recover the hidden plaintext with the correct password.",
    session_id,
)
...
with st.container(border=True):
    ...
    st.caption("Use either a local stego image or a retrieval code generated by the encoder.")

    upload_tab, code_tab = st.tabs(["Upload stego image", "Use retrieval code"])
    ...
    with code_tab:
        st.caption("Retrieve a previously generated encoded image by code.")
...
# -------------------------------
with st.container(border=True):
    st.subheader("Step 2 — Decode settings")
    st.caption("The password and method must match the values used during encoding.")
    ...
    st.selectbox(
        ...
        ["LSB", "DCT", "DWT"],
        key="decoder_method",
        help="Choose the same method used on the encoder page.",
    )
    ...
    if st.session_state["decoder_uploaded_bytes"] is None:
        ...
        try:
            ...
            with st.spinner("Decoding secret from image..."):
                progress.progress(35, text="Extracting embedded payload...")
                progress.progress(70, text="Decrypting plaintext...")
                result = decode_image(
            ...
            progress.progress(100, text="Plaintext recovered.")
...
if decode_result:
    with st.container(border=True):
        st.markdown("<div class='result-banner'><strong>✅ Message decoded successfully.</strong><br>The recovered plaintext is shown below.</div>", unsafe_allow_html=True)
